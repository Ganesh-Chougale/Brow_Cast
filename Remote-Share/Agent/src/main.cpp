    ...
    int main(int argc, char* argv[]) {
        ...
        // The agent now connects to the /agent path on the WebSocket server.
        std::string server_url = "ws://localhost:8080/agent"; 
        ...
        ws_client.setOnCloseHandler([]() {
            std::cerr << "WebSocket disconnected from server. Attempting reconnect..." << std::endl; // Updated message for clarity
        });
            ...
            if (ws_client.isConnected()) {
                ...
                if (!pixel_data.empty() && currentWidth > 0 && currentHeight > 0) {
                    ...
                    if (!jpeg_data.empty()) {
                        ...
                        frame_msg["image"] = ImageProcessor::EncodeToBase64(jpeg_data); // Encoded directly
